    ...
    async def settle(self, payload: dict, requirements: dict) -> dict:
        ...
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            ...
            if verify.status_code >= 400:
                return {"success": False, "errorReason": f"verify {verify.status_code}"}
            verified = verify.json()
            if verified.get("isValid") is False:
                return {
                    "success": False,
                    "errorReason": verified.get("invalidReason") or "payment invalid",
                }
            settle = await client.post(f"{self._base}/settle", json=body)
            if settle.status_code >= 400:
                return {"success": False, "errorReason": f"settle {settle.status_code}"}
            result = settle.json()
            return {

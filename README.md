...
Caps: **5,000** pages, **32,000** characters per page, evicted after **7 days**. Error pages are not stored. Private/internal URLs are rejected. Oldest pages are dropped when the cap is hit. Live size: `GET /v1/stats` (free). Origin `robots.txt` is not honored today.

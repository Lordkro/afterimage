...
Caps: **5,000** pages, **100,000** characters per page, evicted after **10 days**. Error pages, status pages, `noarchive`, and `Vary: *` are not stored. Origin `Cache-Control: no-store` / `private` is indexed for search but never reused as a cache hit. Search hits include `truncated` and collapse identical hashes into `aliases`. Private/internal URLs are rejected. Oldest pages are dropped when the cap is hit. Live size: `GET /v1/stats` (free). Origin `robots.txt` is not honored today.
...
`scripts/x402_ping.py` pays once each for `/v1/search`, `/v1/page`, and both MCP tools so PayAI can catalog them. Zero-hit searches accumulate at `GET /v1/gaps` (a crawl list from real misses).
